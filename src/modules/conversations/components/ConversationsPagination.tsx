...
export default function ConversationsPagination() {
  const dispatch = useAppDispatch();
  const { pagination, filters, isLoading } = useAppSelector(
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-6 border-t border-[var(--admin-border)]">
      ...
      <div className="text-sm text-[var(--admin-text-muted)]">
        ...
        <span className="font-medium text-[var(--admin-text)]">{total}</span> conversations
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-center sm:justify-start">
          <label className="text-sm text-[var(--admin-text-muted)] whitespace-nowrap">
            Per page:
          </label>
          <select
            value={filters.limit}
